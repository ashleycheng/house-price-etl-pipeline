...
class ArgOptions(PipelineOptions):
    @classmethod
    def _add_argparse_args(cls, parser):
        # Use add_value_provider_argument to add the runtime parameter "input"
...
class CreateFn(beam.DoFn):
    # Read value of the runtime parameter
    def __init__(self, input):
        self.input = input

    def process(self, null):
...
def create_dataframe(readable_file):
    ...
    # Remove the row of English header
    dataFrame = dataFrame.drop(labels=0, axis=0)
    # Remove the hidden character
    dataFrame.columns = dataFrame.columns.str.replace('\ufeff', '')
    # Filter land transaction data
    dataFrame = dataFrame[dataFrame.交易標的 == '土地']
...
class data_clean(beam.DoFn):
    ...
    def process(self, element):
        ...
        element['unit_price_m2'] = pd.to_numeric(
            element['unit_price_m2'], errors='coerce')
...
class price_m2_to_ping(beam.DoFn):
    ...
    def process(self, element):
        ...
        element = element.rename({'unit_price_m2': 'unit_price_ping',
                                  'land_area_m2': 'land_area_ping'}, axis=1)
...
def run(argv=None):
    ...
    options = pipeline_options.view_as(ArgOptions)
    ...
    with beam.Pipeline(options=PipelineOptions()) as pipeline:
        files_and_contents = (
            pipeline
            | beam.Create([None])
            | 'Get the File URLs' >> beam.ParDo(CreateFn(options.input))
